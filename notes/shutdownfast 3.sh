...
# Optimize initramfs and GRUB configuration
optimize_initramfs_and_grub() {
    echo_info "Optimizing initramfs and GRUB..."
...
# Add preload to improve startup time
add_preload() {
    echo_info "Adding preload to improve startup time..."
    apt-get install -y preload
    check_status $? "Failed to install preload."
    systemctl enable preload
    systemctl start preload
    echo_info "Preload installed and started."
}
...
# Disable hibernate and suspend features
disable_hibernate_suspend() {
    echo_info "Disabling hibernate and suspend features..."
    systemctl mask sleep.target suspend.target hibernate.target hybrid-sleep.target
    echo_info "Hibernate and suspend features have been disabled."
}
